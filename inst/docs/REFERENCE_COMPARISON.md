...
| Binary LDSC sample size and prevalence | Binary VCF counts were not extracted. | When `POPULATION_PREV` is supplied, uses per-SNP `N=NC+NCO` and derives trait-level `SAMPLE_PREV=NC/(NC+NCO)` from header totals or the unambiguous maximum-total-sample row. | **CRITICAL** | Liability-scale conversion and LDSC regression weights are wrong when prevalence or sample size is wrong. |
