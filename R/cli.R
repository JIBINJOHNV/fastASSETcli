...
parse_munge_column_map <- function(value) {
  ...
  allowed <- c("SNP", "A1", "A2", "EFFECT", "INFO", "P", "N", "MAF", "Z")
...
parse_fastasset_cli <- function(arguments) {
  ...
  required <- c("fastasset-input", "output-dir", "run-name")
  supplied <- vapply(
    required,
  ...
  automatic_required <- c("sumstats-manifest", "hm3", "ld-ref")
...
validate_cli_params <- function(params) {
  ...
  if (params$ldsc_mode == "generate" && !file.exists(params$sumstats_manifest)) {
    missing_files <- c(
      missing_files,
  ...
  if (params$ldsc_mode == "generate") {
    generated_inputs <- c(
      "--sumstats-manifest" = params$sumstats_manifest,
      "--hm3" = params$hm3,
